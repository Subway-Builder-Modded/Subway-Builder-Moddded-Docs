...
can fire multiple times (e.g., when switching cities). The `initialized` guard
...
`modifyConstants()` early — ideally before `onMapReady`. Constants affect the
entire game session.
...
only works in the desktop (Electron) app. In the browser version, `set` does nothing
