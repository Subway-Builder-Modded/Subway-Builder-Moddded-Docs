...
const SOURCE = {
  type: "mods",
  indexUrl:
    "https://raw.githubusercontent.com/Subway-Builder-Modded/The-Railyard/refs/heads/main/mods/index.json",
};
...
function normalizeImageList(manifest, id) {
  ...
  const toUrl = (value) => {
    ...
    if (typeof value === "string") {
      ...
      return `https://raw.githubusercontent.com/Subway-Builder-Modded/The-Railyard/refs/heads/main/mods/${id}/gallery/${value}?raw=true`;
    }
...
export default function RailyardModsPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const loadedItems = await Promise.all(
          ids.map(async (id) => {
            const manifestUrl = `https://raw.githubusercontent.com/Subway-Builder-Modded/The-Railyard/refs/heads/main/mods/${id}/manifest.json`;
            try {
  ...
  return (
    <Layout title="Railyard Mods" description="Browse community Subway Builder mods from Railyard.">
      <main className={styles.page}>
        <section className={styles.hero}>
          <h1>Railyard Mods</h1>
          <p>Search, sort, filter, and download community mods directly from the registry.</p>
          <div className={styles.quickStats}>
            <span>{items.length} mods loaded</span>
        ...
        {isLoading && <p className={styles.status}>Loading mod manifests…</p>}
