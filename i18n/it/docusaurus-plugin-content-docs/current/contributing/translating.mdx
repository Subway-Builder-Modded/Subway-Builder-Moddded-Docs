...
:::important
Throughout this guide, `YOUR_LOCALE_CODE` will refer to the language you are working on. For example, for Spanish, it would be `es`. For English, it would be `en`. For a full list, look [here](https://www.w3schools.com/tags/ref_language_codes.asp). For countries that have multiple regional dialects (like Chinese having `zh-CN` and `zh-TW`), you should use that as `YOUR_LOCALE_CODE`
:::
...
:::important
If you are using a regional dialect of a language that already exists (like Chinese having `zh-CN` and `zh-TW`), you should use that instead of a 2 letter code, like `fr`. Example:
```js
