...
Auf Windows und Linux kannst du [7-Zip](https://www.7-zip.org/download.html) zum Extrahieren
...
Stell sicher, dass es sich um eine `.json` Datei handelt. Wenn auf deinem Betriebssystem die
...
Wenn Dein Verzeichnis mehrere `.js` Dateien beinhaltet, überprüfe alle. Eine davon hat die
...
Wenn Deine Datei kein `initialViewState` enthält, kannst du einfach 12 oder 13 für `zoom`
...
Auch wenn es nicht verpflichtend ist, gib bitte den `creator` an, wenn du die portierte
