...
Windows und Linux kannst du [7-Zip](https://www.7-zip.org/download.html) zum Extrahieren
...
sicher, dass es sich um eine `.json` Datei handelt. Wenn auf deinem Betriebssystem die
...
Dein Verzeichnis mehrere `.js` Dateien beinhaltet, überprüfe alle. Eine davon hat die
...
Deine Datei kein `initialViewState` enthält, kannst Du einfach 12 oder 13 für `zoom`
...
wenn es nicht verpflichtend ist, gib bitte den `creator` an, wenn Du die portierte
