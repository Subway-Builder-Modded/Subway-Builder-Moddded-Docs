...
- Esto llama internamente a `api.reloadMods()`, que limpia todos los callbacks del mod, componentes
...
:::note La recarga en caliente limpia la mayor parte del estado del mod, pero algunas cosas
persisten (como vías y estaciones ya construidas). Si tu mod entra en un estado extraño, reiniciar
el juego completamente es la forma más segura de restablecerlo. :::
...
**Dentro del juego**: Abre la consola de desarrollador con **Ctrl+Shift+I** (Windows/Linux) o
**Cmd+Option+I** (Mac).
...
El script de tu mod se ejecutó antes de que el juego inicializara la API. Esto no debería ocurrir
...
- Verifica que estés registrando la UI dentro de `onMapReady` — el sistema de UI no está listo antes
  de que el mapa cargue
- Confirma que tu componente no lance errores durante el renderizado (revisa la consola para ver
  errores de React)
...
- Verifica que Vite haya reconstruido correctamente (revisa la terminal en busca de errores de
  compilación)
...
- Activar el Modo Desarrollador en la configuración de Windows (Configuración > Para
