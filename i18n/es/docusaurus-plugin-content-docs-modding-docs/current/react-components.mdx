...
Los mods de Subway Builder pueden renderizar UI personalizada usando React. El juego proporciona
React en tiempo de ejecución — tu mod no necesita incluirlo en el bundle.
...
Esto simplemente funciona — el import se redirige al shim en tiempo de compilación y, en tiempo de
ejecución, utiliza la instancia de React del juego.
...
El juego proporciona ~5000 iconos de [Lucide](https://lucide.dev/). Accede a ellos por su nombre en
PascalCase:
...
El juego incluye [Recharts](https://recharts.org/) para visualización de datos. Todos los
componentes estándar están disponibles:
...
El juego utiliza Tailwind CSS. Puedes usar clases utilitarias de Tailwind directamente en tus
componentes:
...
Para una UI más avanzada, puedes registrar un componente de React directamente en una ubicación de
la interfaz:
