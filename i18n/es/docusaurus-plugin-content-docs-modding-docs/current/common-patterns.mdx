...
:::note `onMapReady` puede ejecutarse varias veces (por ejemplo, al cambiar de ciudad). La
...
La forma más común de agregar una UI de mod. Crea un panel arrastrable accesible desde la barra de
herramientas:
...
:::important Llama a `modifyConstants()` temprano — idealmente antes de `onMapReady`. Las constantes
...
:::note El almacenamiento solo funciona en la aplicación de escritorio (Electron). En la versión del
