...
puede ejecutarse varias veces (por ejemplo, al cambiar de ciudad). La
...
a `modifyConstants()` temprano — idealmente antes de `onMapReady`. Las constantes
...
almacenamiento solo funciona en la aplicación de escritorio (Electron). En la versión del
