...
Esta guía cubre cómo contribuir con traducciones al Wiki Modificado de Subway Builder. Los traductores son responsables de asegurarse de que sus contribuciones sean precisas, claras y consistentes con el contenido original. Todas las contribuciones deben seguir la gramática adecuada, mantener el formato y enviarse mediante Pull Request (PR) para revisión.
...
**4.** **No** intentes traducir el contenido de `/blogs`; estos no pueden ser traducidos debido a limitaciones del framework. Sin embargo, en el directorio `docusaurus-plugin-content-blog` puede haber un `options.json`. Este sí debe traducirse.  
**5.** Requisitos de PR:  
