...
**3.** Ensure that all text is properly translated while keeping the formatting intact (headings,
lists, code blocks, notes, tips, admonitions, and images).

**4.** Do **not** attempt to translate the content from `/blogs`; these cannot be translated due to
limitations in the framework. However, in the `docusaurus-plugin-content-blog` directory, there may
be an `options.json`. This should be translated.
...
**6.** Submit your changes via Pull Request. A wiki maintainer will review your submission before
merging.
